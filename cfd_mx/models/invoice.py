...
class AccountInvoiceLine(models.Model):
    ...
    def _compute_price_sat(self):
        ...
        if self.invoice_line_tax_id:
            # taxes = self.invoice_line_tax_id.compute_all(price, self.currency_id, self.quantity, self.product_id, self.partner_id)
            taxes = self.invoice_line_tax_id.compute_all(price, self.quantity, product=self.product_id, partner=self.invoice_id.partner_id)
        self.price_tax_sat = taxes.get('total_included', 0.00) - taxes.get('total', 0.00)
        self.price_subtotal_sat = self.price_unit * self.quantity
        self.price_discount_sat = discount * self.quantity

    price_subtotal_sat = fields.Float(string='Amount (SAT)', store=True, readonly=True, compute='_compute_price_sat', default=0.00)
    price_tax_sat = fields.Float(string='Tax (SAT)', store=True, readonly=True, compute='_compute_price_sat', default=0.00)
    price_discount_sat = fields.Float(string='Discount (SAT)', store=True, readonly=True, compute='_compute_price_sat', default=0.00)
    ...
    def get_impuestos_sat(self):
        ...
        taxes = line.invoice_line_tax_id.compute_all(price_unit, line.quantity, product=line.product_id, partner=line.invoice_id.partner_id)['taxes']
        # taxes = line.invoice_line_tax_id.compute_all(price_unit, line.currency_id, line.quantity, line.product_id, line.invoice_id.partner_id)['taxes']
        for tax in taxes:
            ...
            impuestos = {
                ...
                'Base': '%.2f'%(currency.round(tax['price_unit'] * line['quantity'])),
...
class AccountInvoice(models.Model):
    ...
    @api.depends(
        ...
        'invoice_line.price_subtotal_sat',
        'invoice_line.price_tax_sat',
        'invoice_line.price_discount_sat',
        'tax_line.amount',
        'currency_id',
        'company_id',
        'date_invoice',
    ...
    def _get_parcialidad_pago(self):
        self.parcialidad_pago = len(self.payment_ids) or 0
    ...
    price_subtotal_sat = fields.Float(string='Amount (SAT)', store=True, readonly=True, compute='_compute_price_sat')
    price_tax_sat = fields.Float(string='Tax (SAT)', store=True, readonly=True, compute='_compute_price_sat')
    price_discount_sat = fields.Float(string='Discount (SAT)', store=True, readonly=True, compute='_compute_price_sat')
    ...
    def onchange_metododepago(self):
        ...
        if self.formapago_id and self.formapago_id.banco:
            if not self.partner_id:
                raise Warning("No se ha definido cliente")
    ...
    def _prepare_refund(self, invoice, date=None, period_id=None, description=None, journal_id=None):
        ctx = dict(self.env.context)
        values = super(AccountInvoice, self)._prepare_refund(invoice, date=date, period_id=period_id, description=description, journal_id=journal_id)
    ...
    def action_validate_cfdi(self):
        ...
        for line in self.invoice_line:
            if not line.uos_id.clave_unidadesmedida_id.clave:
                message += '<li>Favor de Configurar la Clave Unidad SAT "%s"</li>'%(line.uos_id.name)
