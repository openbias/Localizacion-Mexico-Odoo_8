...
class ContabilidadElectronicaCodigoAgrupador(models.Model):
    ...
    def _get_accounts_ids(self):
        ...
        for cod in self:
            acc_ids = acc_obj.search([('codigo_agrupador','=', cod.id)])
            account_ids = []
            account_count = 0
            if acc_ids:
    ...
    def get_codigo_agrupador(self):
        ...
        for cod_brw in cod_ids:
            for account in cod_brw.with_context( **context ).mapped('account_ids'):
                signo = 1 
                if account.naturaleza and account.naturaleza.code == "A":
                    signo = -1
                cuenta = {
                    'id': account.id,
...
class ContabilidadElectronicaOtroMetodoPago(models.Model):
    ...
    metodo = fields.Many2one("contabilidad_electronica.metodo.pago", string=u"Método de pago", required=True)
    monto = fields.Float("Monto", required=True)
    fecha = fields.Date("Fecha", required=True)
    benef = fields.Many2one("res.partner", string="Beneficiario", required=True)
    moneda = fields.Many2one("res.currency", string="Moneda")
