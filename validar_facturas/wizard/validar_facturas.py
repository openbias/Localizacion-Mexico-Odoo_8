...
class validar_facturas(models.TransientModel):
    ...
    def get_process_data(self, datas):
        ...
        for inv in self.env["account.invoice"].search(['|', ('uuid', '=', xml_datas.get("uuid")), ('uuid', '=', xml_datas.get("uuid").replace("-",""))]):
            ...
            raise Warning("Ya se tiene en el sistema una factura con el UUID %s - %s "%(xml_datas.get("uuid"), (inv.name or inv.reference)))

        partner_id = self.env['res.partner'].search([('vat', '=', xml_datas.get("rfc_emisor"))], limit=1)
        if not partner_id:
            raise Warning(u"No se encontro en el sistema un proveedor con el RFC %s"%xml_datas.get("rfc_emisor"))

        partner_company_id = self.env['res.partner'].search([('vat', '=', xml_datas.get("rfc_receptor"))], limit=1)
        if not partner_company_id:
            raise Warning(u"El RFC no corresponde a la Empresa %s"%xml_datas.get("rfc_receptor"))
    ...
    def get_invoice_data(self):
        ...
        # print 'sssss', self._cr
        # print 'ssssssss', self.env.user
        # last_account_id = InvoiceLine._default_account()
        # print "desssssssssss", last_account_id
        last_account_id = None
        last_account_id = self.product_id.property_account_expense and self.product_id.property_account_expense.id or self.product_id.categ_id.property_account_expense_categ.id

        data["partner_id"] = self.partner_id.id
        data["account_id"] = self.partner_id.property_account_payable.id
        data["uuid"] = uuid
